...
/*
 * Implements a standalone CLI app that opens iCode from a remote terminal.
 ...
 *  - In external terminals for WSL this calls iCode on the Windows side.
 *    The iCode desktop executable path is passed in env VSCODE_CLIENT_COMMAND.
 */
...
function fatal(message: string, err: unknown): void {
	console.error('Unable to connect to iCode server: ' + message);
	console.error(err);
	process.exit(1);
}
