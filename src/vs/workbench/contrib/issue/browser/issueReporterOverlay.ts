...
export class IssueReporterOverlay {
	...
	private createStep0Attachments(): void {
		...
		subtitle.textContent = localize('screenshotsSubtitle', "You can add up to {0} screenshots or videos. Navigate iCode and choose when to capture.", MAX_ATTACHMENTS);
	...
	private createFloatingCaptureBar(): void {
		...
		// Mount inside .monaco-workbench so iCode's color theme CSS vars
		...
		// Delay/options dropdown using iCode's context menu
	...
	private createStep1Describe(): void {
		...
		// iCode (the most common target). Extension is preselected only when an
	...
	private getIssueTypeOptions(): { type: IssueType; label: string; icon: { id: string } }[] {
		...
		// itself, where performance metrics from a single iCode instance aren't useful.
	...
	private getAllSourceOptions(): { label: string; value: IssueSource }[] {
		return [
			...
			{ label: localize('extensionSource', "An iCode extension"), value: IssueSource.Extension },
	...
	private getIssueSourceLabel(): string {
		switch (this.selectedIssueSource) {
			...
			case IssueSource.Extension:
				return this.selectedExtension?.displayName || this.selectedExtension?.name || localize('extensionSource', "An iCode extension");
			case IssueSource.Marketplace:
	...
	private updateReviewDetails(): void {
			...
			this.createDiagSection(diagContainer, {
				...
				renderContent: (container) => {
					...
					if (modelData.versionInfo) {
						this.addDiagRow(sysTable, 'iCode', modelData.versionInfo.vscodeVersion);
		...
		if (modelData.extensionData) {
			...
			// Extension (e.g. built-in extensions are filed against iCode),
	...
	private generateIssueDetailsMd(): string {
		...
		const rows: [string, string | undefined][] = [
			...
			['iCode Version', modelData.versionInfo?.vscodeVersion ?? product.version],
	...
	private generateSystemInfoMd(): string {
		...
		if (modelData.versionInfo) {
			rows.push(
				['iCode Version', modelData.versionInfo.vscodeVersion],
