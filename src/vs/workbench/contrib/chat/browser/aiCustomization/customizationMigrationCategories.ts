	...
	getCardDescription(customizations, harnessLabel) {
		...
		if (workspaceCount > 0 && userCount > 0) {
			return localize(
				...
				"Prompt files are deprecated for this harness. Found {0} prompt files ({1} workspace, {2} global) that local iCode can still run, but {3} ignores. Convert them to skills to keep them available.",
		...
		if (workspaceCount > 0) {
			return localize(
				...
				"Prompt files are deprecated for this harness. Found {0} workspace prompt files that local iCode can still run, but {1} ignores. Convert them to skills to keep them available.",
				workspaceCount, harnessLabel,
			);
		}
		return localize(
			'promptMigrationCardDescriptionUser',
			"Prompt files are deprecated for this harness. Found {0} global prompt files that local iCode can still run, but {1} ignores. Convert them to skills to keep them available.",
	...
	getPageDescription(customizations, harnessLabel) {
		...
		if (workspaceCount > 0 && userCount > 0) {
			return localize(
				...
				"Prompt files are not supported for this harness. Found {0} prompt files ({1} workspace, {2} user) that local iCode can still run, but {3} ignores. Convert them to skills to keep them available.",
				totalCount, workspaceCount, userCount, harnessLabel,
			);
		}
		if (workspaceCount > 0) {
			return localize(
				...
				"Prompt files are not supported for this harness. Found {0} workspace prompt files that local iCode can still run, but {1} ignores. Convert them to skills to keep them available.",
				workspaceCount, harnessLabel,
			);
		}
		return localize(
			'promptMigrationPageDescriptionUser',
			"Prompt files are not supported for this harness. Found {0} user prompt files that local iCode can still run, but {1} ignores. Convert them to skills to keep them available.",
			userCount, harnessLabel,
		);
	},
	...
	getBanner(_customizations, harnessLabel) {
		return {
			message: localize(
				...
				"Prompts are no longer supported by {0}. Convert them to skills to keep them available in both iCode and this harness.",
				harnessLabel,
			),
		};
	},
	...
	getCardDescription(customizations, harnessLabel) {
		...
		if (agentCount > 0 && instructionsCount > 0) {
			return localize(
				...
				"User data customizations are only used by iCode. Found {0} customizations that {1} ignores. Move them to keep them available.",
		...
		if (agentCount > 0) {
			return agentCount === 1
				? localize(
					...
					"User data customizations are only used by iCode. Found 1 agent that {0} ignores. Move it to keep it available.",
					harnessLabel,
				)
				: localize(
					'userDataMigrationCardDescriptionAgents',
					"User data customizations are only used by iCode. Found {0} agents that {1} ignores. Move them to keep them available.",
		...
		return instructionsCount === 1
			? localize(
				...
				"User data customizations are only used by iCode. Found 1 instruction file that {0} ignores. Move it to keep it available.",
				harnessLabel,
			)
			: localize(
				'userDataMigrationCardDescriptionInstructions',
				"User data customizations are only used by iCode. Found {0} instruction files that {1} ignores. Move them to keep them available.",
		...
		return {
			message: destinationLabel
				? localize(
					...
					"They are stored in user data, which only iCode reads. Move them to '{0}' so both iCode and this harness can use them, keeping their name, type, and content.",
					destinationLabel,
				)
				: localize(
					'userDataMigrationBannerMessage',
					"They are stored in user data, which only iCode reads. Migrating moves them into the folders {0} reads, keeping their name, type, and content, so you can keep using them.",
					harnessLabel,
				),
	...
	getPageDescription(customizations, harnessLabel) {
		...
		if (agentCount > 0 && instructionsCount > 0) {
			return localize(
				...
				"Found {0} customizations in user data that local iCode can still use, but {1} ignores. Move them to the harness folders to keep their type and content.",
				totalCount, harnessLabel,
			);
		}
		if (agentCount > 0) {
			return agentCount === 1
				? localize(
					...
					"Found 1 agent in user data that local iCode can still use, but {0} ignores. Move it to the harness agents folder to keep it available.",
					harnessLabel,
				)
				: localize(
					'userDataMigrationPageDescriptionAgents',
					"Found {0} agents in user data that local iCode can still use, but {1} ignores. Move them to the harness agents folder to keep them available.",
					agentCount, harnessLabel,
				);
		}
		return instructionsCount === 1
			? localize(
				...
				"Found 1 instruction file in user data that local iCode can still use, but {0} ignores. Move it to the harness instructions folder to keep it available.",
				harnessLabel,
			)
			: localize(
				'userDataMigrationPageDescriptionInstructions',
				"Found {0} instruction files in user data that local iCode can still use, but {1} ignores. Move them to the harness instructions folder to keep them available.",
				instructionsCount, harnessLabel,
			);
	},
...
const configuredLocationsMigrationCategory: ICustomizationMigrationCategory = {
	...
	getPageDescription(customizations, harnessLabel) {
		return customizations.length === 0
			...
			: localize('configuredLocationsMigrationPageDescription', "Found {0} customizations in locations configured through iCode settings that {1} does not use. Move them to supported harness locations.", customizations.length, harnessLabel);
	},
	...
	getBanner(_customizations, harnessLabel, destinationLabel, modifiedSettingIds) {
		...
		const message = settingsLinks.length === 1
			? destinationLabel
				? localize('configuredLocationsMigrationBannerSingleSettingWithDestination', "The setting {0} is no longer read by {1}. Move the customizations to '{2}' so both iCode and {1} can use them.", settingsList, harnessLabel, destinationLabel)
				: localize('configuredLocationsMigrationBannerSingleSetting', "The setting {0} is no longer read by {1}. Move the customizations into supported harness folders so both iCode and {1} can use them.", settingsList, harnessLabel)
			: destinationLabel
				? localize('configuredLocationsMigrationBannerSettingsWithDestination', "The settings {0} are no longer read by {1}. Move the customizations to '{2}' so both iCode and {1} can use them.", settingsList, harnessLabel, destinationLabel)
				: localize('configuredLocationsMigrationBannerSettings', "The settings {0} are no longer read by {1}. Move the customizations into supported harness folders so both iCode and {1} can use them.", settingsList, harnessLabel);
		return {
