...
export class ChatPetWidget extends Disposable {
	...
	constructor(
		...
		this._register(this._button.onDidClick(e => {
			...
			if (wasSleeping || this._transientState.get() === 'waking') {
				status(localize('chatPet.wokeUp', "The iCode pet woke up"));
				return;
			}
			...
			switch (interaction) {
				case 'buttonPress':
					status(localize('chatPet.pressedButton', "The iCode pet pressed its button"));
					break;
				case 'complete':
					status(localize('chatPet.spun', "The iCode pet did a rare spin"));
					break;
				case 'love':
					status(localize('chatPet.loved', "The iCode pet feels loved"));
					break;
				case 'cool':
					status(localize('chatPet.cool', "The iCode pet put on sunglasses"));
					break;
				case 'yapping':
					status(localize('chatPet.yapping', "The iCode pet is yapping"));
					break;
				case 'sing':
					status(localize('chatPet.singing', "The iCode pet is singing"));
					break;
				case 'speechless':
					status(localize('chatPet.speechless', "The iCode pet is speechless"));
					break;
				case 'worry':
					status(localize('chatPet.worried', "The iCode pet is worried"));
					break;
			}
		}));
		this._register(this.chatPetService.onDidUnlockAchievement(id => {
			...
			status(localize('chatPet.achievement.unlockedStatus', "Achievement unlocked: {0}. Activate the iCode pet to view achievements.", getChatPetAchievement(id).title));
		}));
	...
	private _completeFall(announce: boolean, wallImpact?: ChatPetWall): void {
		if (this._fallLandsOnPlatform) {
			...
			if (announce) {
				...
				if (respawned) {
					status(localize('chatPet.respawned', "The iCode pet respawned"));
				} else if (shouldCelebrateChatPetBounceScore(this._bounceCount)) {
					status(localize('chatPet.bounceMilestone', "The iCode pet landed with a {0}-bounce streak", this._bounceCount));
				} else if (wallImpact === 'left') {
					status(localize('chatPet.bouncedOffLeftWall', "The iCode pet bounced off the left wall and landed on the chat input"));
				} else if (wallImpact === 'right') {
					status(localize('chatPet.bouncedOffRightWall', "The iCode pet bounced off the right wall and landed on the chat input"));
				} else {
					status(localize('chatPet.landed', "The iCode pet landed on the chat input"));
				}
			}
		...
		if (announce) {
			if (wallImpact === 'left') {
				status(localize('chatPet.bouncedOffLeftWallAndFell', "The iCode pet bounced off the left wall, fell off, and will respawn automatically"));
			} else if (wallImpact === 'right') {
				status(localize('chatPet.bouncedOffRightWallAndFell', "The iCode pet bounced off the right wall, fell off, and will respawn automatically"));
			} else {
				status(localize('chatPet.fellOff', "The iCode pet fell off and will respawn automatically"));
			}
		}
	}
	...
	private _showContextMenu(event: MouseEvent): void {
		...
		const grow = actions.add(new Action('chat.pet.grow', localize('chatPet.grow.action', "Grow"), undefined, true, () => {
			...
			status(localize('chatPet.grew', "iCode pet size: {0} percent", Math.round(scale * 100)));
		}));
		const shrink = actions.add(new Action('chat.pet.shrink', localize('chatPet.shrink.action', "Shrink"), undefined, this._scale > CHAT_PET_MIN_SCALE, () => {
			const scale = getChatPetScale(this._scale, -CHAT_PET_SCALE_STEP);
			this.chatPetService.setScale(scale);
			status(localize('chatPet.shrank', "iCode pet size: {0} percent", Math.round(scale * 100)));
		}));
		const resetSize = actions.add(new Action('chat.pet.resetSize', localize('chatPet.resetSize.action', "Reset Size"), undefined, this._scale !== CHAT_PET_DEFAULT_SCALE, () => {
			...
			status(localize('chatPet.sizeReset', "iCode pet size: {0} percent", CHAT_PET_DEFAULT_SCALE * 100));
		}));
	...
	private _onKeyDown(event: KeyboardEvent): void {
		...
		if (throwRequested && !this._motionReduced) {
			...
			status(direction < 0
				? localize('chatPet.thrownLeft', "The iCode pet was thrown toward the left wall")
				: localize('chatPet.thrownRight', "The iCode pet was thrown toward the right wall"));
			return;
		}
		this._hopController.request(direction, this._motionReduced);
		status(direction < 0
			? localize('chatPet.movedLeft', "iCode pet moved left")
			: localize('chatPet.movedRight', "iCode pet moved right"));
	}
	...
	private _getAriaLabel(onTheRun: boolean, achievementUnlocked: boolean): string {
		return achievementUnlocked
			...
			: onTheRun
				? localize('chatPet.restore', "Bring back the iCode pet")
				: localize('chatPet.interact', "Interact with the iCode pet. Drag it around the chat, or flick it toward either side to throw it. While it is falling, catch it with the pointer to bounce it; while it is airborne, press Enter or Space to bounce it. Use the left and right arrow keys to make it hop, or hold Shift to throw it toward a wall. Use the context menu to put it on the run.");
	}
	...
	private _showRespawnEffect(): void {
		...
		status(localize('chatPet.respawning', "The iCode pet is respawning"));
	}
	...
	private _bounceAirbornePet(pointerX: number, pointerVelocity: ChatPetThrowVelocity, requireDescending = false, collisionMotion?: ChatPetThrowMotion): boolean {
		...
		if (bounced) {
			...
			this._updateBounceCounterPosition();
			status(localize('chatPet.bounceCount', "iCode pet bounce count: {0}", this._bounceCount));
		}
		return bounced;
	}
	...
	private _recordDirectionChange(direction: ChatPetFacingDirection): boolean {
		...
		status(localize('chatPet.dizzy', "The iCode pet got dizzy"));
		return true;
	}
