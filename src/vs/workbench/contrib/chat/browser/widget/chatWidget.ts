...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	render(parent: HTMLElement, petMovementBounds?: HTMLElement, preferredPetHost?: IObservable<boolean>): void {
		...
		// max-width margins in the classic iCode chat view) to the chat list.
	...
	private renderWelcomeViewContentIfNeeded() {
		...
		try {
			...
			if (!numItems) {
				...
				if (this.chatEntitlementService.anonymous && !this.chatEntitlementService.sentiment.completed && product.defaultChatAgent?.provider) {
	...
	private _getGenerateInstructionsMessage(): IMarkdownString {
		...
		if (!this._instructionFilesCheckPromise) {
			...
			// Use iCode's idiomatic pattern for disposal-safe promise callbacks
