...
registerAction2(class extends Action2 {
	...
	run(accessor: ServicesAccessor): void {
		...
		status(localize('chatPet.developer.resetSizeComplete', "iCode pet size reset to {0} percent", Math.round(chatPetService.scale.get() * 100)));
	}
});
...
export class ChatPetAchievementsAccessibilityHelp implements IAccessibleViewImplementation {
	...
	getProvider(_accessor: ServicesAccessor): AccessibleContentProvider {
		...
		const content = [
			...
			localize('chatPet.achievements.accessibilityHelp.roadmap', "The final TBD card is informational and lists upcoming pet ideas. The iCode pet and achievements are experimental and may change."),
