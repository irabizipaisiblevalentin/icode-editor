...
export const ChatPetContextKeys = {
	enabled: new RawContextKey<boolean>('chatPetEnabled', false, localize('chatPet.context.enabled', "Whether the iCode pet is enabled")),
};
...
type ChatPetEnablementClassification = {
	owner: 'justschen';
	comment: 'Tracks iCode pet enablement so adoption can be measured.';
	enabled: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'Whether the iCode pet is enabled.' };
	source: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'Whether the state was observed at startup or changed while iCode was running.' };
};
...
export class ChatPetService extends Disposable implements IChatPetService {
	...
	toggle(): boolean {
		...
		status(enabled
			? localize('chatPet.enabled', "iCode pet enabled. Click the pet to interact with it, or use the Left and Right Arrow keys to move it.")
			: localize('chatPet.disabled', "iCode pet disabled"));
		return enabled;
	}
	...
	setVariant(variant: ChatPetVariant): void {
		...
		status(variant === 'stable'
			? localize('chatPet.variant.stable', "iCode pet changed to the Stable colors")
			: localize('chatPet.variant.insiders', "iCode pet changed to the Insiders colors"));
	}
	...
	setOnTheRun(onTheRun: boolean): void {
		...
		status(onTheRun
			? localize('chatPet.onTheRun', "The iCode pet is on the run. Click the pet to bring it back.")
			: localize('chatPet.restored', "The iCode pet is back"));
	}
