...
export class OutputMonitor extends Disposable implements IOutputMonitor {
	...
	private async _handleIdleState(token: CancellationToken): Promise<{ resources?: ILinkLocation[]; shouldContinuePolling: boolean; output?: string }> {
		...
		// Check for iCode's task finish messages (like "press any key to close the terminal").
		// If the execution is a task and the output contains a iCode task finish message,
		...
		if (isTask && detectsVSCodeTaskFinishMessage(outputTail)) {
			this._logService.trace('OutputMonitor: Idle -> iCode task finish message detected, stopping');
...
export function detectsHighConfidenceInputPattern(cursorLine: string): boolean {
	return [
		...
		// Keep regex literals ASCII to avoid widening the emitted startup bundle.
		/^(?:\s|\x1b\[[0-9;]*m)*\?.*[\u203a\u276f\u25b8\u25b6]\s*$/,
...
/**
 * Localized task finish messages from iCode's terminalTaskSystem.
...
/**
 * Detects iCode's specific task completion messages like:
 * - "Press any key to close the terminal."
 * - "Terminal will be reused by tasks, press any key to close it."
 * These appear when a task finishes and should be ignored if the task is done.
 * Note: These messages may be prefixed with " * " by iCode and may have line wrapping
...
/**
 * Detects generic "press any key" prompts from scripts (not iCode task messages).
...
export function detectsGenericPressAnyKeyPattern(cursorLine: string): boolean {
	// Match "press any key" but exclude iCode task-specific messages
