...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	show(): void {
		...
		this.overlay.setAttribute('aria-label', localize('onboarding.a.aria', "Welcome to iCode"));
		...
		if (this.footerLeft) {
			if (this._isLastStep()) {
				...
				if (!this._footerSignInBtn && !this._userSignedIn) {
					...
					this._footerSignInBtn.textContent = localize('onboarding.sessions.signInNudge', "Sign in to iCode using the Payment Passcode");
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		title.textContent = localize('onboarding.signIn.heroTitle', "Welcome to iCode AI");

		const subtitle = append(contentMain, $('p.onboarding-a-signin-subtitle'));
		subtitle.textContent = localize('onboarding.signIn.heroSubtitle', "Sign in to iCode using the Payment Passcode.");
		...
		copilotDisclaimer.append(localize('onboarding.signIn.disclaimer.prefix', "iCode AI may show "));
		...
		copilotDisclaimer.append(localize('onboarding.signIn.disclaimer.improveSuffix', " suggestions and use your data to improve the product. You can change these settings anytime."));
	}
	...
	private _renderPersonalizeSubtitle(container: HTMLElement): void {
		...
		container.append(
			...
			localize('onboarding.personalize.tip.suffix', " to access all iCode commands."),
		);
	}
	...
	private async _applyKeymap(keymapId: string): Promise<void> {
		...
		if (!keymap?.extensionId) {
			return; // iCode default, nothing to install
		}
