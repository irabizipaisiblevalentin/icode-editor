 ...
 * GenAI). The addon requires a newer glibc than iCode's minimum supported
 ...
 * per-target tarball of the addon + core libraries to iCode's CDN at build
 ...
 * variables (`HTTPS_PROXY`/`HTTP_PROXY`/`ALL_PROXY`, with `NO_PROXY`). iCode's
 ...
 * proxy configured only in iCode is honored here and by the native model
...
async function doEnsure(overrideDir: string, platformKey: string, download: IFoundryLocalRuntimeDownload, token: CancellationToken, onProgress?: FoundryLocalRuntimeProgress): Promise<string> {
	...
	// machine running x64 iCode under emulation, then arm64 iCode). Verify
...
export async function provisionRuntime(overrideDir: string, platformKey: string, urlTemplate: string, version: string, token: CancellationToken): Promise<void> {
	...
	// The cache is shared by the utility processes of every open iCode window,
 ...
 * Linux is gated (on glibc): the prebuilt addon needs GLIBC_2.34, newer than iCode's minimum supported distros, so downloading + loading it there would
 ...
 * mirrors the cache layout). The tarball is published to iCode's CDN by
