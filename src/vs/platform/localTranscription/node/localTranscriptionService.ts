 ...
 * libraries). Derived as a sibling of the model cache dir so both live under iCode's cache home; kept separate from model files since it is versioned by SDK
...
export class LocalTranscriptionService extends Disposable implements ILocalTranscriptionService {
	...
	async start(options: { cacheDir: string; model?: string; language?: string; proxyUrl?: string; noProxy?: string; proxyStrictSSL?: boolean; proxyAuthorization?: string; runtimeUrlTemplate?: string; runtimeVersion?: string }): Promise<void> {
		// Bridge iCode's proxy settings into this process's environment before any
		...
		// proxy, not iCode settings directly).
	...
	/**
	 * Apply iCode's proxy settings as environment variables for this process, so
	 * every download leg (our fetches and the native model download) honors a proxy
	 * configured only in iCode (not in the OS environment):
	...
	private async _ensureModel(cacheDir: string, modelId: string): Promise<IModel> {
		...
		this._modelPromise = (async () => {
			try {
				...
				// from iCode's CDN (per `product.dictationRuntime`) into a
				...
				if (!this._manager) {
					// Store downloaded model files under iCode's cache dir so
				...
				if (!model.isCached) {
					...
					// Bridge iCode cancellation to the AbortSignal the SDK expects.
