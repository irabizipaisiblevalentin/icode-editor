...
export interface ILocalTranscriptionService {
	 ...
	 *
	 * `proxyUrl`/`noProxy` bridge iCode's `http.proxy`/`http.noProxy` settings
	 ...
	 *
	 * `proxyStrictSSL === false` (iCode's `http.proxyStrictSSL`) disables TLS
	 * certificate verification for the JavaScript download legs. `proxyAuthorization`
	 * (iCode's `http.proxyAuthorization`, a `Basic <base64>` value) is folded into
	 ...
	 * runtime (Foundry Local addon + core libraries) is downloaded from iCode's
