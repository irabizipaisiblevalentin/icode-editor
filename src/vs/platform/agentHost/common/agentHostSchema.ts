...
/** iCode setting forwarded into {@link AgentHostDisableRepoInfoTelemetryConfigKey}. */
export const DISABLE_REPO_INFO_TELEMETRY_SETTING_ID = 'chat.advanced.debug.disableRepoInfoTelemetry';

/**
 * Root config key forwarded from the renderer when iCode's
...
/**
 * The iCode setting ID for terminal auto approve enablement. Defined here so
...
/** The iCode setting ID for global auto approve enablement. */
export const GLOBAL_AUTO_APPROVE_SETTING_ID = 'chat.tools.global.autoApprove';

/** The iCode setting ID for per-tool auto-approval eligibility. */
...
/**
 * Root config key forwarded from the renderer when iCode's `chat.autoReply`
...
/**
 * Independently synchronized proxy settings retain their iCode `http.*`
 ...
 * `chat.agentHost.copilotAgent.multiRootEnabled` iCode setting.
 */
 ...
 * the `multipleWorkingDirectories` capability. Mirrors the hidden
 * `chat.agentHost.claudeAgent.multiRootEnabled` iCode setting.
 */
...
/**
 * The iCode setting IDs for terminal auto approve rules. Defined here so
...
export const platformRootSchema = createSchema({
	...
	[AgentHostGlobalAutoApproveEnabledConfigKey]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.globalAutoApproveEnabled.description', "Whether iCode's global auto-approve setting is enabled. When `true`, every tool call is auto-approved, equivalent to a session using Allow all."),
		default: false,
	}),
	...
	[AgentHostAutoReplyEnabledConfigKey]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.autoReplyEnabled.description', "Whether iCode's auto-reply setting is enabled. When `true`, `ask_user` questions are auto-answered instead of blocking on the user, mirroring autopilot mode."),
		default: false,
	}),
