...
export const enum AgentHostConfigKey {
	 ...
	 * `chat.agentHost.allowSignedOutWhenUsable` iCode setting; when unset the
	 ...
	 * by the local iCode client from the workbench `github-enterprise.uri`
...
export const agentHostCustomizationConfigSchema = createSchema({
	...
	[AgentHostConfigKey.DefaultShell]: schemaProperty<string>({
		...
		description: localize('agentHost.config.defaultShell.description', "Absolute path to the shell executable used by host-managed terminals. Normally pushed by the connected iCode client from `terminal.integrated.agentHostProfile.<os>` (falling back to `terminal.integrated.defaultProfile.<os>`); when unset, the agent host falls back to the system shell. Only the path is supported; `args` and `env` from the workbench profile are not piped through yet. The workbench only pushes this for the local agent host — remote agent host operators should set this directly in the remote machine's `agent-host-config.json`."),
	}),
