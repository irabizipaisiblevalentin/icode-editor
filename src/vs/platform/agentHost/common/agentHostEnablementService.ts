...
/** Hidden setting that gates the current-harness indicator for existing Agent Host sessions in the main iCode window. */
...
/** Effective setting or experiment value for the existing-session harness indicator in the main iCode window. */
export const AGENT_HOST_EXISTING_SESSION_HARNESS_PICKER_ENABLED_CONTEXT_KEY = new RawContextKey<boolean>('agentHostExistingSessionHarnessPickerEnabled', false, { type: 'boolean', description: nls.localize('agentHostExistingSessionHarnessPickerEnabled', "Whether existing Agent Host sessions in the main iCode window show the current harness as a disabled picker.") });
...
export interface IAgentHostEnablementService {
	 ...
	 * (`sandbox.enabled`). The runtime owns composing and enforcing that floor; iCode reads it
...
configurationRegistry.registerConfiguration({
	...
	properties: {
			...
			policy: {
				...
				localization: {
					description: {
						...
						value: nls.localize('chat.editor.preferCopilotHarness.policy', "Configure whether iCode uses the Agent Host Copilot SDK instead of the local harness for new editor chat sessions."),
					},
				},
			},
		},
		...
		'chat.editor.localAgent.enabled': {
			type: 'boolean',
			description: nls.localize('chat.editor.localAgent.enabled', "When enabled, shows the iCode local chat harness in the chat picker. This setting is ignored in virtual workspaces, where the local chat harness is always available."),
			default: true,
			tags: ['experimental'],
			experiment: { mode: 'startup' },
		},
		[AgentHostExistingSessionHarnessPickerEnabledSettingId]: {
			type: 'boolean',
			description: nls.localize('chat.editor.agentHost.existingSessionHarnessPicker.enabled', "When enabled, existing Agent Host sessions in the main iCode window show the current harness as a disabled picker."),
			default: false,
			included: false,
			tags: ['experimental'],
		},
