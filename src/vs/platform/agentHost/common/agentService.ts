 ...
 *
 * This is the **workbench** iCode setting id. The workbench registers the
...
// bundled with iCode; users either install codex themselves (typically via
...
// iCode instead of leaking via shell env.
//
...
export interface IAgentHostNetworkDiagnosticsInfo {
	...
	/** iCode `http.*` proxy settings observed by the agent host, keyed by setting id (only those that are set). */
...
export interface IAgentConnection {
	...
	listSessions(): Promise<IAgentSessionMetadata[]>;
	createSession(config?: IAgentCreateSessionConfig): Promise<URI>;
	/** Requires the iCode artifact removal capability advertised by initialize. */
	removeSessionArtifact?(session: URI, artifactId: string): Promise<void>;
	createDetachedWorktree?(session: URI, prompt: string): Promise<{ handle: string; worktree: URI }>;
	claimDetachedWorktree?(handle: string): Promise<void>;
	setDetachedWorktreeArchived?(handle: string, archived: boolean): Promise<void>;
