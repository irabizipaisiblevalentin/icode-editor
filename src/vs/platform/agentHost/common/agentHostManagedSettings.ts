...
/**
 * Compatibility bridge from legacy iCode settings to Copilot SDK managed
 * settings.
 *
 ...
 *   source supplies one, so a lone list from iCode would grant blanket
 ...
 * - **Only what survives translation.** A setting is mapped only when its iCode semantics can be expressed exactly in the SDK's rule grammar. Where
 ...
 * mappings whose iCode behavior already honors user and application values,
...
/**
 * Translates iCode's agent network filter into managed domain rules.
 *
 * Only the blocking half is expressible. iCode denies any domain outside a
...
function contributeNetworkDomainRules(configurationService: IConfigurationService): IAgentHostManagedSettingsPermissions | undefined {
	...
	// iCode's restrictive default: with the filter on and neither list
	...
	for (const pattern of denied) {
		...
		// rule from it. iCode matches on the hostname alone, so a denial written
		...
		// iCode accepts a bare `*` as "every domain"; the SDK expresses that as
...
/**
 * Translates iCode's explicit terminal auto-approve denials into managed
 * shell rules.
 *
 ...
 * iCode escapes `*` as a literal character while the SDK reads it as a
...
const managedPermissionsSettings: readonly IManagedPermissionsSettingMapping[] = [
	...
	// Matches iCode, where a user or application value already suppresses
	...
	// The filter and its lists are evaluated together, and iCode honors a user
