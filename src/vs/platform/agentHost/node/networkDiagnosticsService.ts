...
export class NetworkDiagnosticsService implements INetworkDiagnosticsService {
	 ...
	 * traverses the same proxy / TLS / certificate stack the rest of iCode
	...
	async fetch(url: string): Promise<IAgentHostNetworkFetchResult> {
		...
		// Reachability: a GET through IRequestService, which applies iCode's proxy,
		// strictSSL, and certificate handling — the path the rest of iCode uses.
