...
/** `origin` value written by the iCode extension-host Copilot CLI feature. */
...
/**
 * Whether a marker identifies a chat created by the iCode extension host —
 ...
 * origin-less markers carry iCode-specific properties.
 */
...
export class CopilotAgent extends Disposable implements IAgent {
	...
	private async _ensureClientOnce(): Promise<CopilotClient> {
		...
		const startClient = async () => {
			...
			// Build a clean env for the CLI subprocess, stripping Electron/iCode vars
			...
			// Identify iCode's agent host traffic in CAPI
			...
			// (and the Linux/macOS equivalents). iCode core ships the MXC sandbox binaries
			...
			// Add iCode's built-in ripgrep to PATH so the CLI subprocess can find it.
	 ...
	 * malformed marker, or one that no longer identifies a iCode legacy chat).
	...
	async ensureChatAdopted(chat: URI, context: URI | IAgentChatContext): Promise<IAgentChatAdoptionResult> {
		...
		return this._queueSession(sessionId, async (): Promise<IAgentChatAdoptionResult> => {
			...
			if (!(await this._isExtensionHostCliSession(sessionId))) {
				this._logService.info(`[Copilot] Adoption declined for ${sessionId}: not a legacy extension-host Copilot CLI chat (no iCode marker in its SDK session directory)`);
			...
			// Title precedence mirrors the extension's getSessionTitleImpl: the CLI `name`, then the iCode staged title, then the summary.
			...
			// Seed iCode-layer metadata only — the SDK event log on disk is
	 ...
	 * migrated — no native or non-iCode Copilot session's usage is read or written.
	 */
