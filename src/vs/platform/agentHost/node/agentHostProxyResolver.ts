 ...
 * CAPI proxy through iCode's Electron session before spawning the Copilot SDK.
 *
...
export interface IAgentHostProxyResolver {
	 ...
	 * so the same precedence as the rest of iCode applies: `http.noProxy` →
	 * `http.proxy` setting → `HTTP(S)_PROXY` env vars → the host proxy resolution
	 * that runs in iCode (Electron session) via the reverse channel.
	 */
	...
	private _getProxyResolver(): ReturnType<typeof createProxyResolver> {
		if (!this._proxyResolver) {
			...
			const params: ProxyAgentParams = {
				// The host proxy resolution runs in iCode: reverse-call a connected
