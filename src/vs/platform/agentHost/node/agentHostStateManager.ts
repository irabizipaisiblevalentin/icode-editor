...
export class AgentHostStateManager extends Disposable {
	...
	/** Whether the session is owned by a throwaway iCode chat surface. */
	isEphemeralSession(session: string): boolean {
		const entry = this._sessionStates.get(session);
		return entry ? readEphemeralSessionMeta(entry.state).isEphemeral === true : false;
	}

	/** Returns the typed iCode surface metadata for a tracked session, when present. */
	 ...
	 * the session's single chat. iCode models every session as having
	 ...
	 * dispatching this action; for iCode-internal clients that happens
	 ...
	 * activity status) onto the chat channel. To preserve iCode's
