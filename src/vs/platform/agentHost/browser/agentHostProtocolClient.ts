...
// AHP 0.9 changed the automation catalog wire shape, so iCode cannot safely negotiate 0.8.
...
export class AgentHostProtocolClient extends Disposable implements IAgentConnection, IRemoteAgentHostProtocolClient {
	...
	private _handleMessage(msg: ProtocolMessage): void {
		...
		if (isJsonRpcRequest(msg)) {
			...
			switch (msg.method) {
				case 'action': {
					// Protocol envelope → iCode envelope (superset of action types)
	...
	/** Send a JSON-RPC request for a iCode extension method (not in the protocol spec). */
