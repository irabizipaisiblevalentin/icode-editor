...
/**
 * Resolves the iCode remote authority for the given session provider,
 * e.g. `ssh-remote+myhost` or `tunnel+myTunnel`.
 *
 * Returns `undefined` for local or WebSocket-only providers where no
 * iCode remote extension can handle the connection.
 */
...
/** Resolves an Agent Host folder to the URI understood by its iCode remote extension. */
