...
export interface ISessionsRecentWorkspacesService {
	...
	/** Whether iCode's recent-folder and workspace-file history has loaded; Agents-owned history is synchronous. */
	 ...
	 * first, then (when `includeVSCodeRecents` is `true`, the default) iCode's recently opened folders and workspace-file folders (deduplicated against own history).
	 *
	 ...
	 * considering iCode's recently opened folders.
	 */
	...
	/** Removes `folderUri` from the recent list, wherever it came from (own history or iCode's recents). */
