...
/**
 * Auto-trusts every isolated git worktree in `workspace` that iCode created off
 ...
 *   not prove iCode created the worktree (its contract permits an arbitrary
...
export async function ensureSessionWorktreesTrusted(
	...
	await Promise.all(workspace.folders.map(async folder => {
		...
		// root), but on its own it does not prove iCode created that worktree.
