...
export class RemoteAgentHostContribution extends Disposable implements IWorkbenchContribution {
	 ...
	 * Resolves tokens via the standard iCode authentication service.
	 *
...
Registry.as<IConfigurationRegistry>(ConfigurationExtensions.Configuration).registerConfiguration({
	properties: {
		...
		[RemoteAgentHostsSettingId]: {
			...
			description: nls.localize('chat.remoteAgentHosts', "A list of WebSocket remote agent host addresses to connect to (e.g. \"localhost:3000\"). SSH remote agent host details are managed by iCode."),
			default: [],
			scope: ConfigurationScope.APPLICATION,
			restricted: true,
			tags: ['experimental', 'advanced'],
